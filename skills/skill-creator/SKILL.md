---
name: skill-creator
description: Create or update shared Codex and Claude Code skills with appropriately scoped instructions and supporting resources.
metadata:
...
Create skills that give Codex and Claude Code useful, non-obvious guidance without constraining unrelated work.

## Shared repository policy for Leonardo

This installation uses one canonical repository for personal skills shared by Codex and Claude Code:

- Stable checkout: `C:/Projetos TI/agent-skills`.
- Stable skill sources: `C:/Projetos TI/agent-skills/skills/<skill-name>/`.
- Git remote: `https://github.com/leohfurlan/agent-skills.git`.
- Discovery directories are installation links, not independent authoring locations.

For every skill creation or update, inspect this repository's instructions, Git status, worktrees and tags first. Unless the user explicitly requests another repository or project-local scope, keep the skill and all its resources in this repository's `skills/<skill-name>/` tree. An unavailable checkout, missing write permission or conflicting local edits is a blocker to report; never silently fall back to `~/.codex/skills`, `~/.claude/skills` or `~/.agents/skills`.

### Development and promotion

The canonical checkout is loaded by both agents. A branch switch there changes the live skill. Create or reuse a separate worktree of the same repository for development, on a dedicated branch (default prefix `codex/`). New skills belong in `<development-worktree>/skills/<skill-name>/`; this is still the same Git repository, not a second source. Run the initializer with that explicit `--path`. Keep the stable checkout and its junctions unchanged while developing. A user-authorized bootstrap or direct live fix is an explicit exception; disclose it.

Validate structure, inspect references and permissions, and exercise relevant behavior in both agents before proposing promotion. Distinguish checks actually run from review and pending runtime tests. Review the diff and record compatibility changes. Commit/tag/push and promotion require the user's authorization; creating a skill alone does not authorize publishing it. On an authorized release, tag the reviewed commit (for example `<skill-name>/v1.0.0`), update the stable checkout without overwriting unrelated work, then register any newly released skill. Never point global installations at a development worktree. Missing release evidence keeps a draft unpromoted.

### Shared installation

Register each stable skill with a Windows directory junction at `C:/Users/Leonardo/.agents/skills/<skill-name>` for Codex and `C:/Users/Leonardo/.claude/skills/<skill-name>` for Claude Code, targeting the stable source above. Keep the top-level discovery directories real, and leave unrelated system/plugin skills alone. Compare existing content and preserve backups outside the scanned skill directories before replacing a directory or link; stop on differing content rather than overwriting it. Check both link targets and file hashes afterward. Creating a new source folder alone does not register it for both agents.

The shared `skill-creator` is a deliberate exception in Codex: its existing system entry at `C:/Users/Leonardo/.codex/skills/.system/skill-creator/SKILL.md` forwards to this canonical file, so do not also register a duplicate `skill-creator` in `.agents/skills`. Claude uses the normal junction. An application update may replace the system forwarding entry; verify and restore that entry from this policy when authorized rather than editing a divergent creator copy.

Keep shared instructions portable: use relative supporting-resource paths and identify agent-specific tools or metadata explicitly. Sharing files does not install MCP tools or grant permissions. Do not edit the same skill concurrently through both agents.
...
Ask clarifying questions only when the missing information matters and cannot be reasonably inferred. Apply the Shared repository policy above: author in a development worktree of the canonical repository and register only approved stable releases.
...
```bash
scripts/init_skill.py my-skill --path "<development-worktree>/skills"
scripts/init_skill.py my-skill --path "<development-worktree>/skills" --resources references
```
